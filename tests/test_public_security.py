...
def test_health_is_honest_without_database(client):
    ...
    assert response.get_json()["release"] == "2026-09-07-stream-recovery"
...
def test_daily_tool_and_preferences_routes_are_registered(client):
    ...
    assert {
        ...
        "/api/conversations/<int:conversation_id>/messages/stream",
        "/api/messages/<int:message_id>/study-progress",
        "/api/logout-all",
    } <= routes
