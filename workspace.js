...
(function(){
  ...
  function buildDialog(){
    ...
    for(const input of [ui.title,ui.content])input.addEventListener('input',()=>{state.dirty=true;ui.discard.hidden=false;ui.counter.textContent=ui.content.value.length+' / 10,000';status('Unsaved changes');ui.delete.textContent='Delete note';saveNoteDraft()});
  ...
  async function deleteNote(){
    ...
    try{await state.api('/api/quick-notes/'+state.selected.id,{method:'DELETE',body:JSON.stringify({version:state.selected.version})});window.SaathiRecovery?.removeDraft('note',noteDraftKey());state.notes=state.notes.filter(note=>note.id!==state.selected.id);state.dirty=false;state.busy=false;selectNote(null,true);status('Note deleted.')}catch(error){status(error.message,true)}finally{state.busy=false;ui.delete.disabled=false;ui.delete.textContent='Delete note'}
  }
